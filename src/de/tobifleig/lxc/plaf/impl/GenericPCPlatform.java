...
public class GenericPCPlatform implements Platform {
	...
	// Find out if updating is allowed, requested, forced and/or if the verification should be disabled.
	...
	if (Configuration.containsKey("allowupdates")) {
	    ...
	    if ("yes".equals(result.toLowerCase()) || "true".equals(result.toLowerCase())) {
	...
	for (String s : args) {
		...
		// whoever launched LXC tells us he is able to restart us in case of an update
		restartable = true;
	    }
	}
	if (checkForUpdates || forceUpdate) {
	    System.out.println("Checking for Updates...");
	    // Ugly workaround, anonymous inner classes require (local) variables to be final
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(new JFrame(), "Cannot write to own folder. Please move to your home directory or start as administrator.", "Error", JOptionPane.ERROR_MESSAGE);
	    System.exit(1);
	...
	try {
	    ...
	    while (iter.hasNext()) {
		String s = iter.next();
		writer.append(s.toString() + "=" + Configuration.getStringSetting(s) + '\n');
	    }
	} catch (IOException ex) {
	...
	if (!Configuration.containsKey("defaulttarget")) {
	    ...
	    // check if valid
	    File file = new File(Configuration.getStringSetting("defaulttarget"));
